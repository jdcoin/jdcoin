...
void PrepareShutdown()
{
    ...
    util::ThreadRename("jdcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the JDC and zJDC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 5550, 51474));
    ...
    strUsage += HelpMessageOpt("-jdcstake=<n>", strprintf(_("Enable or disable staking functionality for JDC inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zjdcstake=<n>", strprintf(_("Enable or disable staking functionality for zJDC inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:5550"));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 5551, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The JDCOIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("jdcoin-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. JDCOIN Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. JDCOIN Core is probably already running."), strDataDir));
        ...
        do {
            ...
            try {
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zjdc if already reindexed
                    RecalculateJDCSupply(1, fReindexZerocoin);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_JDCOIN && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_JDCOIN);
            }
        ...
        zwalletMain = new CzJDCWallet(pwalletMain);
        ...
        if (!zwalletMain->GetMasterSeed().IsNull()) {
            //Inititalize zJDCWallet
            uiInterface.InitMessage(_("Syncing zJDC wallet..."));

            //Load zerocoin mint hashes to memory
            pwalletMain->zjdcTracker->Init();
