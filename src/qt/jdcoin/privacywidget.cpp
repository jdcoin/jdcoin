    ...
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zJDC anonymizes your JDC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zJDC or convert back to JDC"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zJDC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zJDC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zJDC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zJDC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zJDC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zJDC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zJDC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zJDC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zJDC Movements"));
    ...
    ui->btnTotalzJDC->setTitleClassAndText("btn-title-grey", "Total 0 zJDC");
    ui->btnTotalzJDC->setSubTitleClassAndText("text-subtitle", "Show denominations of zJDC owned.");
    ui->btnTotalzJDC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select JDC outputs to mint into zJDC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zJDC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzJDC, &OptionButton::clicked, this, &PrivacyWidget::onTotalZjdcClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zJDC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of JDC to mint into zJDC"));
    } else {
        btnText = tr("Convert back to JDC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zJDC to convert back into JDC"));
    }
...
void PrivacyWidget::onTotalZjdcClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzJDC->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzJDC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zJDC is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zJDC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zJDC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZjdc(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zJDC converted back to JDC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zJDC </b>";
    ...
    ui->btnTotalzJDC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
