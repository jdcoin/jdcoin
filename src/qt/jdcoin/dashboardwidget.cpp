    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of JDC and zJDC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareJdc, "square-chart-jdc");
    setCssProperty(ui->labelSquarezJdc, "square-chart-zjdc");
    setCssProperty(ui->labelJdc, "text-chart-jdc");
    setCssProperty(ui->labelZjdc, "text-chart-zjdc");
    ...
    ui->labelAmountZjdc->setText("0 zJDC");
    ui->labelAmountJdc->setText("0 JDC");
    setCssProperty(ui->labelAmountJdc, "text-stake-jdc-disable");
    setCssProperty(ui->labelAmountZjdc, "text-stake-zjdc-disable");
    ...
    ui->btnHowTo->setText(tr("How to get JDC or zJDC"));
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &JDCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &JDCOINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &JDCOINGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isJdc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZJDC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair JDC, zJDC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            jdc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zjdc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalJdc += pair.first;
            chartData->totalZjdc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesJdc.append(jdc);
        chartData->valueszJdc.append(zjdc);

        int max = std::max(jdc, zjdc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1->append(chartData->valueszJdc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalJdc > 0 || chartData->totalZjdc > 0) {
        setCssProperty(ui->labelAmountJdc, "text-stake-jdc");
        setCssProperty(ui->labelAmountZjdc, "text-stake-zjdc");
    } else {
        setCssProperty(ui->labelAmountJdc, "text-stake-jdc-disable");
        setCssProperty(ui->labelAmountZjdc, "text-stake-zjdc-disable");
    }
    forceUpdateStyle({ui->labelAmountJdc, ui->labelAmountZjdc});
    ui->labelAmountJdc->setText(GUIUtil::formatBalance(chartData->totalJdc, nDisplayUnit));
    ui->labelAmountZjdc->setText(GUIUtil::formatBalance(chartData->totalZjdc, nDisplayUnit, true));
