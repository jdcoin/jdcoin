    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezJdc, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzJdc, ui->labelTitleImmaturezJdc};
    ...
    setCssProperty({ui->labelAmountTopJdc, ui->labelAmountTopzJdc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountJdc, ui->labelAvailablezJdc}, "amount-topbar");
    setCssProperty({ui->labelPendingJdc, ui->labelPendingzJdc, ui->labelImmatureJdc, ui->labelImmaturezJdc}, "amount-small-topbar");
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // JDC Total
    CAmount jdcAvailableBalance = balance;
    // zJDC Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalJdc = GUIUtil::formatBalance(jdcAvailableBalance, nDisplayUnit);
    QString totalzJdc = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    ...
    // Update display state and/or values for zJDC balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zJDC label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzJdc->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzJdc->setText(totalzJdc);
    // Expanded
    ui->labelAvailablezJdc->setText(totalzJdc);
    ui->labelPendingzJdc->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezJdc->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
