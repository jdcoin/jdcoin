    ...
    btnContacts(new QPushButton())
{
    ...
    ui->pushRight->setText("zJDC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (JDC) or private coins (zJDC)"));
    ...
    setCssProperty(coinIcon, "coin-icon-jdc");
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send JDC") : tr("Send zJDC"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZjdc = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZjdc));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend JDC (not zJDC)
    const bool isZjdc = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZjdc && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZjdc))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    if ((sendJdc) ? send(recipients) : sendZjdc(recipients)) {
...
bool SendWidget::sendZjdc(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zJDC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zJDC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZJdcControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZJdcControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZjdc(
        ...
        inform(tr("zJDC transaction sent!"));
        ZJdcControlDialog::setSelectedMints.clear();
        clearAll(false);
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZJDC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zJDC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zJDC transaction
        ...
        Q_EMIT message("zJDC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isJDC) {
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ZJdcControlDialog *zJdcControl = new ZJdcControlDialog(this);
            zJdcControl->setModel(walletModel);
            zJdcControl->exec();
            ui->btnCoinControl->setActive(!ZJdcControlDialog::setSelectedMints.empty());
            zJdcControl->deleteLater();
        } else {
            inform(tr("You don't have any zJDC in your balance to select."));
        }
    }
}
...
{
    isJDC = _isJDC;
    setCssProperty(coinIcon, _isJDC ? "coin-icon-jdc" : "coin-icon-zjdc");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress jdcAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(jdcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(jdcAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
