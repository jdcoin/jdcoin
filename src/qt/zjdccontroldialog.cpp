...
#include "ui_zjdccontroldialog.h"
...
std::set<std::string> ZJdcControlDialog::setSelectedMints;
std::set<CMintMeta> ZJdcControlDialog::setMints;

bool CZJdcControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZJdcControlDialog::COLUMN_DENOMINATION || column == ZJdcControlDialog::COLUMN_VERSION || column == ZJdcControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZJdcControlDialog::ZJdcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZJdcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zJDC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZJdc->setProperty("cssClass", "text-main-purple");
    ui->labelZJdc_int->setProperty("cssClass", "text-main-purple");
    ...
    // click on checkbox
    connect(ui->treeWidget, &QTreeWidget::itemChanged, this, &ZJdcControlDialog::updateSelection);
    // push select/deselect all button
    connect(ui->pushButtonAll, &QPushButton::clicked, this, &ZJdcControlDialog::ButtonAllClicked);
}

ZJdcControlDialog::~ZJdcControlDialog()
{
    delete ui;
}

void ZJdcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJdcControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZJdcControlWidgetItem* itemDenom(new CZJdcControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZJdcControlWidgetItem *itemMint = new CZJdcControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zJDC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zJDC seed used to mint this zJDC is not the same as currently hold in the wallet";
            else
...
void ZJdcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJdcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZJdc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZJdcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZJdcControlDialog::GetSelectedMints()
{
...
void ZJdcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
