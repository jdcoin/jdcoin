...
bool CJdcStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CJdcStake from zerocoin spend");
...
bool CJdcStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CJdcStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CJdcStake::ContextCheck(int nHeight, uint32_t nTime)
{
