...
UniValue setstakesplitthreshold(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 JDC each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in JDC).\n"
...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zJDC balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zJDC balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nList all zJDC mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zJDC ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zjdcTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zJDC ID", m.hashPubcoin.GetHex()));     // PubCoin
            ...
            if (m.hashStake.IsNull()) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zjdcTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zjdcTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zJDC mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nMint the specified zJDC amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Jdc to convert to zJDC\n"
    ...
    if (!Params().IsRegTestNet())
        throw JSONRPCError(RPC_WALLET_ERROR, "zJDC minting is DISABLED");
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zJDC is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZjdcSpend(nAmount, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nSpend zJDC mints to a JDC address.\n" +
            HelpRequiringPassphrase() + "\n"
...
extern UniValue DoZjdcSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzJDCTracker* zjdcTracker = pwalletMain->zjdcTracker.get();
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzJDCTracker* zjdcTracker = pwalletMain->zjdcTracker.get();
    std::set<CMintMeta> setMints = zjdcTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zJDC\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zJDC serial hash \n"
            ...
            "    \"v\": version,   (numeric) The version of the zJDC\n"
            "    \"k\": \"privkey\"  (string) The zJDC private key (V2+ zJDC only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzJDCTracker* zjdcTracker = pwalletMain->zjdcTracker.get();
    std::set<CMintMeta> setMints = zjdcTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw std::runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zJDC value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zjdcTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zJDC list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzjdcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw std::runtime_error(
            "setzjdcseed \"seed\"\n"
            "\nSet the wallet's deterministic zjdc seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zjdc seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzjdcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzjdcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzJDCWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzjdcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "getzjdcseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zJDC seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzjdcseed", "") + HelpExampleRpc("getzjdcseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zJDC seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zJDC to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zJDC to generate.\n"
    ...
    CzJDCWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzjdcstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "dzjdcstate\n"
                        "\nThe current state of the mintpool of the deterministic zJDC wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzjdc_count", nCount));
...
void static SearchThread(CzJDCWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZJDC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzjdc(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw std::runtime_error(
            "searchdzjdc\n"
            "\nMake an extended search for deterministically generated zJDC that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zJDC to start with.\n"
            "2. \"range\"       (numeric) How many zJDC to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzjdc", "1, 100, 2") + HelpExampleRpc("searchdzjdc", "1, 100, 2"));
    ...
    boost::thread_group* dzjdcThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzjdcThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzjdcThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zjdcTracker->GetSerialHashes());
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZjdcSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
