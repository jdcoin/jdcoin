...
/**
 * Fees smaller than this (in ujdc) are considered zero fee (for transaction creation)
        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zJDC seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZJDC = GetBoolArg("-zapwallettxes", false);
    if (fCheckZJDC)
        zjdcTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zjdc
            if (fCheckZJDC && pindex->nHeight >= Params().GetConsensus().height_start_ZC) {
