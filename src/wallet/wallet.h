...
#include "zjdc/zjdcmodule.h"
#include "zjdc/zjdcwallet.h"
#include "zjdc/zjdctracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_JDCOIN = 61000, // inherited version..
...
// Possible states for zJDC send
enum ZerocoinSpendStatus {
    ZJDC_SPEND_OKAY = 0,                            // No error
    ZJDC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZJDC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZJDC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZJDC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZJDC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZJDC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZJDC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZJDC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZJDC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZJDC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZJDC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZJDC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZJDC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZJDC_SPENT_USED_ZJDC = 14,                      // Coin has already been spend
    ZJDC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZJDC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZJDCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zJDC wallet
    CzJDCWallet* zwalletMain{nullptr};
    std::unique_ptr<CzJDCTracker> zjdcTracker{nullptr};
    void setZWallet(CzJDCWallet* zwallet);
    CzJDCWallet* getZWallet();
    ...
    // zJDC reset
    boost::signals2::signal<void()> NotifyzJDCReset;
