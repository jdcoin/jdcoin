...
public:
    ...
    bool WriteZJDCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZJDCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZJDCSeed_deprecated(uint256& seed);
    bool EraseZJDCSeed();
    bool EraseZJDCSeed_deprecated();

    bool WriteZJDCCount(const uint32_t& nCount);
    bool ReadZJDCCount(uint32_t& nCount);
