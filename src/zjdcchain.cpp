...
std::string ReindexZerocoinDB()
{
    ...
    while (pindex) {
        ...
        // update supply
        UpdateZJDCSupplyConnect(block, pindex, true);
...
{
    CDataStream serializedCoinSpend = ZJDCModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZJDCSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zJDC supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zJDC supply
    ...
    // A one-time event where only the zJDC supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZJDCSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zJDC supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zJDC supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
