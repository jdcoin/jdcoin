...
bool CLegacyZJdcStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZJdcStake from non zc-spend");
    ...
    *this = CLegacyZJdcStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zjdc stake origin", __func__);

    // All good
    return true;
}

CLegacyZJdcStake::CLegacyZJdcStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZJdcStake::GetIndexFrom()
{
...
CAmount CLegacyZJdcStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZJdcStake::GetUniqueness() const
{
...
bool CLegacyZJdcStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zJDC stake block: height %d outside range", __func__, nHeight);
