...
CzJDCWallet::CzJDCWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zjdc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZJDCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZJDCSeed_deprecated()) {
                    LogPrintf("%s: Updated zJDC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zjdc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zjdc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzJDCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZJDCCount(nCountLastUsed);
    else if (!walletdb.ReadZJDCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzJDCWallet::Lock()
{
    seedMaster.SetNull();
}

void CzJDCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzJDCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZJDC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzJDCWallet::LoadMintPoolFromDB()
{
...
void CzJDCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzJDCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzJDCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zjdcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzJDCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZJDC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zjdcTracker which also adds to database
    wallet->zjdcTracker->Add(dMint, true);
...
void CzJDCWallet::SeedToZJDC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzJDCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzJDCWallet::GenerateDeterministicZJDC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzJDCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzJDCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzJDCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
