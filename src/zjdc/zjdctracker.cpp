...
#include "zjdc/zjdcwallet.h"


CzJDCTracker::CzJDCTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzJDCTracker::~CzJDCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzJDCTracker::Init()
{
...
bool CzJDCTracker::Archive(CMintMeta& meta)
{
...
bool CzJDCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzJDCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzJDCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzJDCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzJDCTracker::GetSerialHashes()
{
...
CAmount CzJDCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzJDCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzJDCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzJDCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzJDCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzJDCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzJDCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzJDCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzJDCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzJDCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzJDCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzJDCWallet* zJDCWallet)
{
    bool iszJDCWalletInitialized = (nullptr != zJDCWallet);
    CMintMeta meta;
    ...
    if (!iszJDCWalletInitialized)
        zJDCWallet = new CzJDCWallet(wallet);
    meta.isSeedCorrect = zJDCWallet->CheckSeed(dMint);
    if (!iszJDCWalletInitialized)
        delete zJDCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzJDCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzJDCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzJDCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzJDCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzJDCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzJDCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzjdc from DB\n", __func__, listDeterministicDB.size());
    }
