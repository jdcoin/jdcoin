...
class ReorgStakeTest(JdcoinTestFramework):
    ...
    def check_money_supply(self, expected_jdc, expected_zjdc):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected JDC and zJDC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_jdc))
            for denom in node['zJDCsupply']:
                assert_equal(node['zJDCsupply'][denom], DecimalAmt(expected_zjdc[denom]))
