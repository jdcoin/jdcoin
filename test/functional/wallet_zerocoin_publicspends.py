...
class ZerocoinSpendTest(JdcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zjdc_bal, jdc_bal):
            zjdc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zjdc_bal)
            jdc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], jdc_bal)
            return zjdc_bal, jdc_bal
        ...
        zjdc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zjdc_balance, 6666)
        ...
        zjdc_balance, balance = check_balances(denom_2, zjdc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zjdc_balance, balance = check_balances(denom_3, zjdc_balance, balance)
        ...
        _, _ = check_balances(denom_1, zjdc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
